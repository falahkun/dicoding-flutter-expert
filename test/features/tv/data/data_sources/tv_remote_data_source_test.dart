...
void main() {
  ...
  group('package:tv/src/data_sources/tv_remote_data_source.dart', () {
    group('getNowPlaying', () {
    ...
    group('getTopRated', () {
      test('when success', () async {
        final tTvList = TvResponse.fromJson(
                json.decode(readJson('dummy_data/tv_rop_rated.json')))
            .tvList;
        when(() => client.get(Uri.parse('$BASE_URL/tv/top_rated?$API_KEY')))
            .thenAnswer((_) async =>
                http.Response(readJson('dummy_data/tv_rop_rated.json'), 200));
    ...
    group('Search', () {
      test('when success', () async {
        final tTvList = TvResponse.fromJson(
            json.decode(readJson('dummy_data/tv_search_dasada.json')))
            .tvList;
        when(() => client.get(Uri.parse('$BASE_URL/search/tv?$API_KEY&query=$tQuery')))
            .thenAnswer((_) async =>
            http.Response(readJson('dummy_data/tv_search_dasada.json'), 200));
